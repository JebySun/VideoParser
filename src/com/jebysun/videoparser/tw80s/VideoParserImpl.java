...
//			e.printStackTrace();
		...
		int pageStartIndex = commentList.size();
		Elements nextPageElements = doc.select("#paginator a.next");
		if (!nextPageElements.isEmpty()) {
			String nextPageUrl = nextPageElements.first().attr("href");
			pageStartIndex = Tw80sUtil.getPageStartFromUrl(nextPageUrl);
		}
		
		return new DoubanCommentPage(commentList, pageStartIndex);
	}
	...
	private String buildQueryUrl(String queryUrl, Map<String, String> param) {
		...
		//查询URL响应速度优化（避免重定向再次请求）
		if (queryUrl.endsWith("-p1")) {
			queryUrl = queryUrl.replaceFirst("-p1", "");
		}
		if (queryUrl.equals("/movie/list/----")) {
			queryUrl = "/movie/list";
		} else if (queryUrl.equals("/ju/list/----0-")) {
			queryUrl = "/ju/list";
		} else if (queryUrl.equals("/zy/list/----4-")) {
			queryUrl = "/zy/list";
		} else if (queryUrl.equals("/dm/list/----14-")) {
			queryUrl = "/dm/list";
		}
